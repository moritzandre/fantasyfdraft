...
/** Chip in the unified mode/filter row — lv-seg-on when active. */
const segChip = (on: boolean) =>
  `num min-h-14 shrink-0 rounded-lg border px-4 text-[15px] font-semibold ${
    on ? 'lv-seg-on' : 'border-app-border bg-app-surface text-app-text'
  }`;
...
export default function PlanBSheet({ s, rawBoard }: { s: DraftState; rawBoard: Board }) {
  ...
  const tierRow = (p: any, wash: string) => {
    ...
    return (
      <div
        class={`lv-row lv-tier-${wash} flex items-center gap-1.5 px-2 ${
          gone ? 'lv-struck' : ''
        }`}
      >
        <span
  ...
  // Lighter header: letter chip + one line ("RB · Tier 2 · 6"), cliff/edited
  // as small dim right-aligned notes — no borders, the surface bg + the
  // rows' shared tier edge give the card its shape (restraint rules).
  const tierCard = (pos: string, g: any) => {
    const wash = washOf(g.tier);
    return (
      <section class="mb-4 break-inside-avoid overflow-hidden rounded-xl bg-app-surface">
        <header class="flex min-h-11 items-center gap-2 px-2 py-1.5">
          ...
          <span class="min-w-0 flex-1 truncate text-[15px] font-bold">
            {pos} · Tier {g.tier}
            <span class="num font-normal text-app-dim"> · {g.members.length}</span>
          </span>
          {g.edited && (
            <span
              class="shrink-0 text-xs text-app-dim"
          ...
          {Number.isFinite(g.cliffPoints) && (
            <span class="num shrink-0 text-xs text-app-dim">cliff {fmtInt(g.cliffPoints)} pts</span>
          )}
        </header>
  ...
  const lensRow = (row: { p: any; likelyRound: number; m: number; lens: number; rank: number }) => {
    ...
    return (
      <div
        class={`lv-row flex items-center gap-1.5 px-2 ${
          gone ? 'lv-struck' : ''
        }`}
      >
  ...
  return (
    <div class="lv-planb lv-tool-draft pb-6 text-app-text">
      {/* ONE compact sticky header: back · title (+ dim inline stat) · print */}
      <div class="lv-blurbar sticky top-0 z-10 border-b border-app-border">
        <div class="flex items-center gap-2 px-3 py-2">
          <a
            href="#/live"
            class="flex h-14 min-w-14 shrink-0 items-center justify-center rounded-lg text-xl font-bold"
            aria-label="Back to Live"
          >
            ←
          </a>
          <h1 class="lv-h-tool min-w-0 flex-1 truncate text-xl font-extrabold">
            Plan B{' '}
            <span class="num align-middle text-sm font-normal text-app-dim">
              {takenBy.size} gone
            </span>
          </h1>
          <button
            type="button"
            class={`flex h-14 min-w-14 shrink-0 items-center justify-center rounded-lg text-xl ${
              showPrint ? 'text-accent' : 'text-app-dim'
            }`}
            aria-label="Print kit"
      ...
      {/* ONE scrollable chip row: TIERS/STRATEGIES segment · divider dot ·
          contextual chips (positions in tiers mode, strategies otherwise) */}
      <div class="lv-hscroll items-center gap-1.5 px-3 py-2">
        <button type="button" class={segChip(mode === 'tiers')} onClick={() => setMode('tiers')}>
          TIERS
        </button>
        <button type="button" class={segChip(mode === 'strats')} onClick={() => setMode('strats')}>
          STRATEGIES
        </button>
        <span class="shrink-0 px-0.5 text-app-dim" aria-hidden="true">
          ·
        </span>
        {mode === 'tiers'
          ? ['ALL', ...POS].map((c) => (
              <button
                ...
                class={
                  posFilter === c && c !== 'ALL'
                    ? `lv-posc lv-posc-${c.toLowerCase()} num min-h-14 shrink-0 rounded-lg border border-transparent px-4 text-[15px] font-semibold`
                    : segChip(posFilter === c)
                }
          ...
          : metas.map((m) => (
              <button
                type="button"
                class={segChip(stratName === m.name)}
      ...
      {mode === 'tiers' && (
        <div class={posFilter === 'ALL' ? 'gap-x-4 px-3 md:columns-2' : 'mx-auto max-w-2xl px-3'}>
          {tierGroups
      ...
      {mode === 'strats' && (
        <div class="mx-auto max-w-3xl">
          {/* Compact summary line — plan & rules expand INLINE on tap; the
              target LIST below stays the default visible content. */}
          <button
            type="button"
            class="flex min-h-14 w-full items-center gap-2 px-4 text-left"
            aria-expanded={showPlan}
            onClick={() => setShowPlan(!showPlan)}
          >
            <span class="min-w-0 truncate text-[15px] font-bold">{strat.label ?? strat.name}</span>
            <span class="shrink-0 text-sm font-semibold text-app-dim">
              Plan &amp; rules {showPlan ? '▾' : '▸'}
            </span>
          </button>

          {showPlan && (
            <div class="mx-3 mb-3 rounded-xl bg-app-surface p-3">
              ...
              {(multChips.length > 0 || conChips.length > 0) && (
                <div class="mt-2 flex flex-wrap gap-1">
                  ...
                  {conChips.map((t) => (
                    <span class="lv-tag-warn num rounded px-1.5 py-0.5 text-xs font-semibold">
                      {t}
                    </span>
                  ))}
                </div>
              )}
              <div class="lv-hscroll mt-2 gap-1 px-0.5 pb-1">
                {plan.map((c) => (
                  <div
                    class={`num flex min-w-[4.25rem] shrink-0 flex-col items-center rounded-lg px-1 py-1 text-center ${
                      c.long ? 'lv-tag-warn' : 'bg-app-bg'
                    }`}
                  >
              ...
              <p class="pt-1 text-[11px] text-app-dim">
                amber = long-gap wait (κ {planCfg.kappaLongGap}) · ⊘ = max-blocked now · ×m =
                multiplier in force · ≥ = min-constraint deadline
              </p>
            </div>
          )}
          ...
          {bands.map((b) => {
            ...
            return (
              <section class="mx-3 mb-4 overflow-hidden rounded-xl bg-app-surface">
                <header class="flex items-baseline gap-2 px-3 pb-1 pt-2">
          ...
          <p class="px-4 pb-2 text-xs text-app-dim">
            lens = eff × m(pos, ADP-likely round) — raw points, so compare within a band, not
            across; drafted players stay listed, struck.
          </p>
        </div>
      )}
    </div>
